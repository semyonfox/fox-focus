...
Google Calendar and Microsoft connections remain read-only. The current Google connection requests the full `https://www.googleapis.com/auth/tasks` scope for approved task creation, completion, and reopening. Its two calendar scopes remain read-only. Existing Google connections with only `tasks.readonly` need to reconnect. A reconnect creates a new opaque connection generation, and any older import that finishes late is discarded. See [Provider connections](integrations.md).
