...
The Tasks scope permits the two narrow write paths enforced by Fox Focus: create an approved task and complete or reopen an approved task. A connection that has only `tasks.readonly` must reconnect. The server does not expose title edits, task moves, list clears, task deletes, calendar writes, or bulk completion.
